...
if __name__ == '__main__':
    ...
    parser.add_argument('-wclip','--weight_clipping', type=float, default=5, help="weight clipping", required=False)
    ...
    for i in range(int(args.toys)):
        ...
        if not os.path.isfile(f"{mydir}/{joblabel}.txt"):
            ...
            script_src.write(f"python {os.getcwd()}/{args.pyscript} -o {mydir}/{joblabel} -t {int(args.toys)} -sig {args.signal} -bkg {args.background} -ref {args.reference} -epochs {args.epochs} -latsize {args.latsize} -layers {args.layers} -wclip {args.weight_clipping}") #/{joblabel}
