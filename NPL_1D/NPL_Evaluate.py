...
from Data_Reader import BuildSample, BuildDistribution
...
parser.add_argument('-sig','--signal', type=int, default=1000, help="number of signal events", required=False)
parser.add_argument('-bkg','--background', type=int, default=4000, help="number of background events", required=False)
parser.add_argument('-ref','--reference', type=int, default=50000, help="number of reference events", required=False)
...
# INPUT_PATH_SIG = '/lustre/cmswork/nlai/NPL_CODE/1D/TEST/sig_bkg.txt' 
...
total_epochs = args.epochs # number of epochs (a lot)
latentsize = args.latsize  # number of nodes in each hidden layer (what is this?)
...
# define output path
ID = (
        '/1D_patience'+str(patience)+'_ref'+str(N_Ref)+'_bkg'+str(N_Bkg)+'_sig'+str(N_Sig)
        +'_epochs'+str(total_epochs)+'_latent'+str(latentsize)+'_layers'+str(layers)+'_wclip'+str(weight_clipping)
)
...
OUTPUT_FILE_ID = '/1D_patience'+str(patience)+'_ref'+str(N_Ref)+'_bkg'+str(N_Bkg)+'_sig'+str(N_Sig)+'_toy'+str(toy)
...
# HLF_SIG = BuildSample(FILE_NAME=INPUT_PATH_SIG, n=N_Bkg_p+N_Sig_p)
# print('Signal DF shape:')
# print(HLF_SIG.shape)
...
class myCallback(callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        if epoch % int(total_epochs/50) == 0:
...
hist = BSMfinder.fit(feature, target, batch_size=batch_size, epochs=total_epochs,verbose=2,callbacks=[myCallback()],)
