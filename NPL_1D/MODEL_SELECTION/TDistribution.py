'''
HO MESSO CHE I GRADI DI LIBERTA DEL TEST DEL CHI2 SONO SOLAMENTE IL NUMERO DI BIN, NON PIU' NBINS-1 !!!!!!!!
'''
...
class TDist:
    ...
    def plotOutPath(self):
        ...
        path = '/lustre/cmswork/nlai/PLOTS/test/'
...
#         self.t_list_history = self.t_list_history[self.t_list_history[:, -1]<40]
    ...
    def plotChi2Compatibility(self):
        ...
        self.t_chi2_compatibility = np.array(self.t_chi2_history[:])/(self.bins)
    ...
    def plotChi2Compatibility2(self):
        ...
        # calcolo la compatibilità come chi_nu/nu
        self.t_chi2_compatibility =  np.abs( np.array(self.t_chi2_history[:]) - (self.bins) )  / np.sqrt(2*(self.bins))
    ...
    def plotPValHistory(self):
        ...
        self.pvalue_history = scipy.stats.chi2.sf(self.t_chi2_history[:], df=self.bins) 
